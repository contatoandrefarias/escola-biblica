...
def inicializar_banco():
    ...
    # Tabela de Alunos (AGORA COM AS NOVAS COLUNAS DIRETAMENTE NO CREATE TABLE)
    cursor.execute("""
    ...
    # --- Lógica de Migração para adicionar novas colunas (ainda mantida para segurança,
    # mas o CREATE TABLE acima já deve resolver para novos DBs) ---
    ...
    if 'nome_pai' not in colunas_alunos:
        ...
        print("Coluna 'nome_pai' adicionada à tabela 'alunos' via ALTER TABLE.")
    if 'nome_mae' not in colunas_alunos:
        cursor.execute("ALTER TABLE alunos ADD COLUMN nome_mae TEXT")
        print("Coluna 'nome_mae' adicionada à tabela 'alunos' via ALTER TABLE.")
    if 'endereco' not in colunas_alunos:
        cursor.execute("ALTER TABLE alunos ADD COLUMN endereco TEXT")
        print("Coluna 'endereco' adicionada à tabela 'alunos' via ALTER TABLE.")

    # Adicionar colunas de notas específicas à tabela matriculas se não existirem
    ...
    if 'desafio_nota' not in colunas_matriculas:
    ...
    conn.commit()
    conn.close() # Fechar a conexão após a inicialização
