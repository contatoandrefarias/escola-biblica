...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "escola_biblica_chave_2026")
...
def novo_aluno():
    if request.method == "POST":
        ...
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO alunos (nome, telefone, email, data_nascimento, membro_igreja) VALUES (?, ?, ?, ?, ?)",
...
def editar_aluno(id):
    ...
    if request.method == "POST":
        ...
        email = request.form.get("email", "").strip()
        membro = 1 if request.form.get("membro_igreja") else 0
        cursor.execute("UPDATE alunos SET nome=?, telefone=?, email=?, membro_igreja=? WHERE id=?",
...
def novo_professor():
    if request.method == "POST":
        ...
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO professores (nome, telefone, email, especialidade) VALUES (?, ?, ?, ?)",
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            INSERT INTO disciplinas
                (nome, descricao, carga_horaria, nota_minima, frequencia_minima, professor_id)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (nome, descricao, int(carga), float(nota_min), float(freq_min), prof_id))
        conn.commit()
        conn.close()
...
def nova_matricula():
    if request.method == "POST":
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO matriculas (aluno_id, disciplina_id) VALUES (?, ?)",
                (aluno_id, disc_id))
            conn.commit()
    ...
    conn.close()
    return render_template("nova_matricula.html", alunos=alunos_lista, disciplinas=disc_lista)
...
def lancar_notas(id):
    ...
    if request.method == "POST":
        ...
        cursor.execute("UPDATE matriculas SET nota1=?, nota2=?, nota_final=?, status=? WHERE id=?",
...
def presenca():
    ...
    if request.method == "POST":
        ...
        for m in matriculados:
            ...
            if existe:
                cursor.execute("UPDATE presencas SET presente=? WHERE id=?",
                    (presente, existe["id"]))
            else:
                cursor.execute("INSERT INTO presencas (matricula_id, data_aula, presente) VALUES (?, ?, ?)",
...
def relatorios():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT a.nome as aluno, d.nome as disciplina, m.nota_final, m.status
        FROM matriculas m
        JOIN alunos a ON m.aluno_id = a.id
        JOIN disciplinas d ON m.disciplina_id = d.id
    ...
    conn.close()
    return render_template("relatorios.html", matriculas=todas, ranking=ranking)
